...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM students WHERE roll_no = ? LIMIT 1";
    ...
    if ($result->num_rows == 1) {
        ...
        // ✅ Verify password (plain text for now, but you can use password_hash)
        if ($row['password'] === $password) {
            $_SESSION['roll_no'] = $row['roll_no']; // Store roll_no in session
        ...
        echo "<script>alert('❌ Roll number not found'); window.history.back();</script>";
    }
}
?>
