...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($check->num_rows > 0) {
        echo "⚠️ Roll Number already registered!";
    } else {
        // Hash password
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);

        $stmt = $conn->prepare("INSERT INTO students (roll_no, password) VALUES (?, ?)");
        $stmt->bind_param("ss", $roll_no, $hashed_password);
...
<body>
    <form method="POST" action="register.php" id="register-form" class="register-form" autocomplete="off" role="main">
        <h1 class="a11y-hidden">Register Form</h1>

        <!-- Roll No -->
        <div>
            <label class="label-email">
                <input type="text" class="text" name="roll_no" placeholder="Roll No" tabindex="1" required />
                <span class="required">Roll No</span>
            </label>
        </div>
